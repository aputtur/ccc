...
public abstract class BasePaymentAction extends Action
{
    ...
    try{
	    ...
	    LdapUser ldapUser = user.getLdapUser();
	    ...
	    if (user.getBillingAddress() != null) {
	        ...
	        address2 = user.getBillingAddress().getAddress2();
	        city = user.getBillingAddress().getCity();
	        state = user.getBillingAddress().getState();
	        zip = user.getBillingAddress().getPostalCode();
	    	...
	    	address1 = ldapUser.getMailToLine1();
	    	address2 = ldapUser.getMailToLine2();
	    	city = ldapUser.getMailToCity();
	    	state = ldapUser.getMailToStateProvince();
	    	zip = ldapUser.getMailToPostalCode();
	    	country = ldapUser.getMailToCountryCode();
	    	userid = user.getDisplayName();
	   	 	email = ldapUser.getUsername();
			phone_number = ldapUser.getMailToPhoneNumber();
	    }
	    if(address2 == null){
	    	address2 = "";
	    }
	    ...
	    frm.setAddress1(address1);
	    //cartForm.setAddress2(StringUtils.defaultString(address2));
	    frm.setAddress2(address2);
	    frm.setCity(city);
	    frm.setState(state);
	    frm.setZip(zip);
	    frm.setCountry(country);
	    frm.setUserName(userid);
	    frm.setUserEmail(email);
	    frm.setUserPhone(phone_number);
...
//		String key = "CS_HOP_" + ccAuthReply_reasonCode;
             ...
             HttpServletRequest req,HttpServletResponse resp) throws IOException
    {
    	 ...
    	 String currencyAmount = req.getParameter("totalAmountInUSD").trim();
    	 ...
    	 try{
             ...
             if(results.size() == 1){
                 ...
                 while (currencyRates.hasNext()){
                	 ...
                	 //currencyAmount = paymentForm.getCartChargeTotal().substring(1).trim();
