...
Fields:
	...
	-submitDetailId: current detail id
...
<#if showLink == true >
<li <#if parameters.hrefId?exists>id="${parameters.hrefId}_li_${item.ID?c}"</#if> >
		...
		<a class="editDetailAngleTabs" <#if parameters.hrefId?exists>id="${parameters.hrefId}_${item.ID?c}" </#if>
		 ...
		 onclick="submitDetailTopMenuForm_${parameters.submitInstance?c}_${parameters.submitDetailId?c}('<@s.url action="${parameters.submitAction}" 
        ...
        <#else>
        	...
        	<a class="editDetailAngleTabs" <#if parameters.hrefId?exists>id="${parameters.hrefId}_${item.ID?c}" </#if>
        	 href="#" <#if parameters.hrefTarget?exists>target="${parameters.hrefTarget}"</#if>
       	     onclick="submitDetailTopMenuForm_${parameters.submitInstance?c}_${parameters.submitDetailId?c}('<@s.url value="${parameters.submitValue}" includeParams="none"/>');return false;">
        	${parameters.hrefValue?html}
        	</a><span><img align="top" border="0" width="19" height="26" src="<@s.url value="/resources/ordermgmt/images/spacer.gif"/>"></img></span>
       		</#if> 	
        </#if>
