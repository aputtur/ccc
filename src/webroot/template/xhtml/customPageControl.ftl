...
Fields:
	...
	-hideLast: hides last button for >1000 results
-->


<#assign pageControl="${parameters.pagingObject}">
<#assign objId="${parameters.pagingObject?replace('.','_')}">
<#assign newPageFieldId="${parameters.pagingFormId}_newPage">
...
<#else>
	<#assign submitForm="doRePageResults">
</#if>
...
<#assign totalRecords=stack.findValue('${pageControl}.totalRecords')>

<#if !parameters.inFooter?? || parameters.inFooter!='true'>
	<@s.hidden theme="simple" cssClass="${sNoClearClass}" name="newPage" value="${currentPage}"/>
	...
	<@s.hidden theme="simple" cssClass="${sNoClearClass}" name="${pageControl}.totalRecords"/>
	<@s.hidden theme="simple"cssClass="${sNoClearClass}" name="${pageControl}.pageSizeChanged"/>
</#if>

<#if (listSize > 0)>
	<#if (currentPage * pageSize > listSize && listSize == totalRecords)>
		<#assign endRecordNum=listSize>
	<#else>
		<#assign endRecordNum=(currentPage * pageSize)>
	</#if>
	<b>Results:&nbsp;showing&nbsp;</b>
	${((currentPage-1) * pageSize + 1)} - ${endRecordNum} of ${totalRecords}
	...
	<#if !parameters.hideLast?? || parameters.hideLast!='true'>
		<#if (totalPages > 1)>
	...
	<#else>
		<#if (totalPages > 1 && totalRecords <= 1000)>
		    <#if (currentPage != totalPages)>
		       <#if (endPage != totalPages)>
		    		&nbsp;
					<a href="#" onclick="document.getElementById('${newPageFieldId}').value='${totalPages?c}';${submitForm};return false;">Last</a>
